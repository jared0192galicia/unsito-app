...
export default function NoticiaPage({ params }: NoticiaPageProps) {
  ...
  return (
    <div className="bg-app-white min-h-screen">
      ...
      <div className="flex w-full p-10 h-40 absolute justify-between text-app-blue-700 text-2xl">
        <ArrowLeft
          className={cn(
            'text-xl hover:bg-app-blue-600/50 p-2 w-10 h-10 rounded-full',
            'hover:animate-pulse cursor-pointer transition-colors duration-300',
            '',
          )}
          onClick={() => router.back()}
        />
        <ArrowRight
          className={cn(
            'text-xl hover:bg-app-blue-600/50 p-2 w-10 h-10 rounded-full',
            'hover:animate-bounce cursor-pointer transition-colors duration-300',
            '',
          )}
        />
      </div>
        ...
        <article className="w-full max-w-5xl bg-white p-6 md:p-10">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="relative">
              <img
                src={note.banner || '/images/banner.png'}
                alt={note.title}
